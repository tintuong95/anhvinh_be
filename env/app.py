...
app.config['MYSQL_DATABASE_DB'] = 'quanly'
    ...
    def post(self):
        try:
            ...
            cursor.execute(taoThietBiCmd, (
                _tenthietbi, _ngaynhap,_ngaysx,_hsx,_loai,_giaban,_soluong,_daban))
            conn.commit()
            response = jsonify(
...
# Danh sách routers
api.add_resource(DanhSachThietBi, '/thietbis', endpoint='thietbis')
api.add_resource(ThietBi, '/thietbi/<int:thietbi_id>', endpoint='thietbi')
api.add_resource(DanhSachDanhMuc, '/danhmucs', endpoint='danhmucs')
